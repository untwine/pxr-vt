...
#ifndef PXR_VT_WRAP_ARRAY_H
#define PXR_VT_WRAP_ARRAY_H
...
Vt_ConvertFromPySequenceOrIter(TfPyObjWrapper const &obj)
{
    ...
    if (PySequence_Check(obj.ptr())) {
        ...
        for (Py_ssize_t i = 0; i != len; ++i) {
            pxr::boost::python::handle<> h(PySequence_ITEM(obj.ptr(), i));
            if (!h) {
                if (PyErr_Occurred())
                    PyErr_Clear();
                return VtValue();
            }
            pxr::boost::python::extract<ElemType> e(h.get());
            if (!e.check())
                return VtValue();
            *elem++ = e();
        }
        ...
        while (PyObject *item = PyIter_Next(obj.ptr())) {
            pxr::boost::python::handle<> h(item);
            if (!h) {
                if (PyErr_Occurred())
                    PyErr_Clear();
                return VtValue();
